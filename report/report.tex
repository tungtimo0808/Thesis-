...
\begin{figure}[H]
    ...
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        stage/.style={draw, line width=1pt, rounded corners=4pt, fill=white, align=center,
                      text width=2.7cm, minimum height=2cm, font=\small,
                      drop shadow={shadow xshift=0.5pt, shadow yshift=-0.7pt, opacity=0.25}},
        arr/.style={-{Stealth[length=3mm]}, line width=1.1pt, black!45},
        node distance=0.75cm,
    ]
        \node[stage, draw=cblue]                  (s1) {\textbf{\textcolor{cblue}{Panoramic image}}\\[3pt]{\scriptsize full view or\\regional crop}};
        \node[stage, draw=cgreen,  right=of s1]   (s2) {\textbf{\textcolor{cgreen}{Vision encoder}}\\[3pt]{\scriptsize ViT $\rightarrow$\\image tokens}};
        \node[stage, draw=corange, right=of s2]   (s3) {\textbf{\textcolor{corange}{Connector}}\\[3pt]{\scriptsize MLP into the\\LLM input space}};
        \node[stage, draw=cpurple, right=of s3]   (s4) {\textbf{\textcolor{cpurple}{Language model}}\\[3pt]{\scriptsize reads tokens\\and schema prompt}};
        \node[stage, draw=cmagenta, right=of s4]  (s5) {\textbf{\textcolor{cmagenta}{JSON report}}\\[3pt]{\scriptsize parsed into\\tooth findings}};
        \draw[arr] (s1) -- (s2);
        \draw[arr] (s2) -- (s3);
        \draw[arr] (s3) -- (s4);
        \draw[arr] (s4) -- (s5);
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[->] (img) -- (vit);
        \draw[->] (vit) -- node[above, font=\footnotesize] {image tokens} (llm);
        \draw[->] (llm) -- (out);
    \end{tikzpicture}}
...
\begin{figure}[H]
    ...
    \caption{Per-condition F1 for every model (conditions sorted by test support). The common conditions \texttt{H} and \texttt{R} are read well; the rare conditions stay close to zero.}
