...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[box] (img) {Panoramic image\\(full view or crop)};
        \node[box, right=1cm of img] (enc) {Vision\\encoder (ViT)};
        \node[box, right=1cm of enc] (conn) {Connector\\(MLP)};
        \node[box, right=1cm of conn] (llm) {Language\\model};
        \node[box, right=1cm of llm] (json) {JSON report\\(parsed)};
        \node[draw, dashed, rounded corners, align=center, font=\footnotesize, above=0.8cm of llm] (prompt) {Schema\\prompt};
        \draw[->] (img) -- (enc);
        \draw[->] (enc) -- node[above, font=\footnotesize] {image tokens} (conn);
        \draw[->] (conn) -- (llm);
        \draw[->] (llm) -- (json);
        \draw[->] (prompt) -- (llm);
    \end{tikzpicture}}
