...
Later methods used attention and Transformer-based models. R2Gen is one example. It used a memory-based Transformer design for radiology report generation and tested it on IU X-Ray and MIMIC-CXR \cite{chen2020r2gen}. This type of work shows that report generation benefits from both image features and language patterns. A radiology report often uses repeated medical phrases, so the model needs to learn both visual evidence and report style. More recent work moves from task-specific decoders to large language models. R2GenGPT, for example, aligns the image features to a frozen large language model and reaches strong results on chest X-ray report datasets \cite{wang2023r2gengpt}. This is the same shift, from a special decoder to a general language model, that this thesis follows for dental reports.
...
Faster R-CNN is a common baseline for object detection. In dental panoramic X-rays, it can detect tooth boxes, tooth numbers, or dental condition classes. Tuzoff et al. used Faster R-CNN for tooth detection and reported high detection performance on panoramic radiographs \cite{tuzoff2019tooth}. Deep learning has also been used to read tooth conditions, not only tooth positions. For example, Oztekin et al. detected dental caries on panoramic radiographs with a convolutional neural network and reported high accuracy \cite{oztekin2023caries}. Some systems already do both parts together: Y\"uksel et al. (DENTECT) number the teeth in FDI notation and detect several dental treatments on the same panoramic image with one deep-learning model \cite{yuksel2021dentect}. This is close to the recognition part of the present task, although it uses a detector and does not produce a written report. It shows that both the tooth and its condition can be learned from panoramic images, which is also what this thesis asks the models to do.
...
\begin{figure}[H]
    \centering
    \definecolor{cblue}{HTML}{2F6DB0}
    \definecolor{cgreen}{HTML}{3AA856}
    \definecolor{corange}{HTML}{E08E0B}
    \definecolor{cpurple}{HTML}{6A4C9C}
    \definecolor{cmagenta}{HTML}{B0338C}
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        stage/.style={rounded corners=4pt, align=center, text=white,
                      text width=2.7cm, minimum height=2cm, font=\small,
                      drop shadow={shadow xshift=0.6pt, shadow yshift=-0.8pt, opacity=0.3}},
        arr/.style={-{Stealth[length=3.5mm]}, line width=1.4pt, black!45},
        node distance=0.8cm,
    ]
        \node[stage, fill=cblue]                  (d1) {\textbf{PAN924 data}\\[3pt]{\scriptsize images and\\JSON annotations}};
        \node[stage, fill=cgreen,  right=of d1]   (d2) {\textbf{Clean labels}\\[3pt]{\scriptsize FDI number\\and condition}};
        \node[stage, fill=corange, right=of d2]   (d3) {\textbf{Create five views}\\[3pt]{\scriptsize full image and\\four regional crops}};
        \node[stage, fill=cpurple, right=of d3]   (d4) {\textbf{Build samples}\\[3pt]{\scriptsize prompt, JSON answer,\\image path}};
        \node[stage, fill=cmagenta, right=of d4]  (d5) {\textbf{Export data}\\[3pt]{\scriptsize VLM JSONL\\and COCO}};
        \draw[arr] (d1) -- (d2);
        \draw[arr] (d2) -- (d3);
        \draw[arr] (d3) -- (d4);
        \draw[arr] (d4) -- (d5);
        \node[anchor=west, font=\large\bfseries] (dttl) at ([yshift=1.55cm]d1.north west) {Data Preparation Pipeline};
        \node[anchor=west, font=\small\color{black!60}] at ([yshift=1.0cm]d1.north west) {From the PAN924 annotations to the training files.};
        \node[anchor=west, font=\footnotesize\color{black!55}] at ([yshift=-0.85cm]d1.south west) {The same source data is reused for both VLM report generation and Faster R-CNN detection.};
    \end{tikzpicture}}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[stage, fill=cblue]                  (s1) {\textbf{Panoramic image}\\[3pt]{\scriptsize full view or\\regional crop}};
        \node[stage, fill=cgreen,  right=of s1]   (s2) {\textbf{Vision encoder}\\[3pt]{\scriptsize ViT $\rightarrow$\\image tokens}};
        \node[stage, fill=corange, right=of s2]   (s3) {\textbf{Connector}\\[3pt]{\scriptsize MLP into the\\LLM input space}};
        \node[stage, fill=cpurple, right=of s3]   (s4) {\textbf{Language model}\\[3pt]{\scriptsize reads tokens\\and schema prompt}};
        \node[stage, fill=cmagenta, right=of s4]  (s5) {\textbf{JSON report}\\[3pt]{\scriptsize parsed into\\tooth findings}};
        \draw[arr] (s1) -- (s2);
        \draw[arr] (s2) -- (s3);
        \draw[arr] (s3) -- (s4);
        \draw[arr] (s4) -- (s5);
        \node[anchor=west, font=\large\bfseries] (ttl) at ([yshift=1.55cm]s1.north west) {Inference Workflow: From X-ray to Report};
        \node[anchor=west, font=\small\color{black!60}] at ([yshift=1.0cm]s1.north west) {A trained model turns a panoramic image into a structured report.};
        \node[anchor=west, font=\footnotesize\color{black!55}] at ([yshift=-0.85cm]s1.south west) {Only the vision encoder, connector and language model change between models; the prompt, schema and parsing stay the same.};
    \end{tikzpicture}}
...
\begin{itemize}
    ...
    \item \textbf{Clinical pilot.} The web interface built in this work could be tested with a dentist in the loop, to measure how useful the draft report is in practice.
\end{itemize}
