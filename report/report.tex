...
\end{center}
    ...
    \vspace{1cm}
    {\huge \textbf{BACHELOR THESIS}}\\[0.75cm]
...
\textbf{Student Name} & Nguyen Hoang Tung \\
\textbf{Student ID}   & 22BA13318 \\
\textbf{Supervisor}   & Nghiem Thi Phuong \
\end{tabular}
}
\end{center}
...
\begin{center}
    \textbf{\large SUPERVISOR'S APPROVAL FOR BACHELOR THESIS}
\end{center}
...
Topic: & Vision language models for automatic dental report generation \\
Specialty: & Data Science \\
Student's full name: & Nguyen Hoang Tung \\
Student's ID No: & 22BA13318 \\
Supervisor's full name: & Nghiem Thi Phuong \\
Working place: & Information and Communication Technology \\
\end{tabularx}

\vspace{1cm}

\noindent
I hereby agree with the quality of this Bachelor thesis for student Nguyen Hoang Tung. This report is qualified for the graduation thesis defense.

\vspace{1cm}

\begin{flushright}
Hanoi, \ldots\ldots\ldots July, 2026
\end{flushright}

\noindent
\hfill
\makebox[5cm][center]{\textbf{Supervisor}}
...
I would like to express my sincere thanks to my supervisor, Nghiem Thi Phuong, for her guidance throughout this thesis. From the early planning to the final writing, she gave clear advice, asked the right questions, and helped me keep the work focused and realistic. Her feedback shaped both the direction of the project and the way the results are presented, and she was patient whenever I needed to rethink an idea or fix a mistake. I am very grateful for the time, encouragement, and support she gave at every stage of this work.
\newpage
...
\begin{itemize}
    \item Build a clean instruction dataset from the PAN924 panoramic dental X-ray dataset, based only on the ground-truth labels, that is, the FDI tooth numbers and the dental condition codes.
    \item Create five image views for each original image, one full panoramic image and four regional crops, so the model can learn from both the whole mouth and the close-up details.
    \item Generate report-style comments and summaries from fixed clinical templates, without adding any finding that is not already in the annotations.
    \item Validate the dataset and convert it into the input format of the four model families, so every model trains and is tested on the same split.
    \item Address the strong class imbalance, so the rare dental conditions are not ignored during training.
    \item Fine-tune the five models with the same QLoRA setup, so any difference in the results comes from the model and not from the training recipe.
    \item Evaluate the models on a held-out test set with one shared metric script, and compare them on tooth detection, condition classification, and report-text quality.
\end{itemize}
...
This section reviews three lines of related work: generating reports from medical images, vision language models for medical reporting, and detection on dental panoramic X-rays.
...
\subsection{Vision Language Models for Medical Reporting}
Vision Language Models take an image and a text instruction as input, and return a text answer. This instruction-following ability has been applied in medicine, for report generation and visual question answering \cite{hartsock2024vlmreview}. In dentistry, the direction is very recent. Liu et al. tested GPT-4o, a large proprietary VLM of undisclosed size, on panoramic radiographs, asking it to report common dental conditions at specific teeth using the FDI and Universal numbering systems \cite{liu2025dentalvlm}. It handled some conditions well, such as dental implants and restorations, but failed on small lesions, scoring zero F1 on caries. That study used a single closed model in a zero-shot way on fifty images. This thesis differs: it fine-tunes several open models, compares them under one fixed setup, and generates a full structured report rather than only flagging conditions.
...
\subsection{Tooth and Condition Detection on Panoramic X-rays}
Faster R-CNN is a common baseline for object detection. In dental panoramic X-rays, it can detect tooth boxes, tooth numbers, or dental condition classes. Tuzoff et al. used Faster R-CNN for tooth detection and reported high detection performance on panoramic radiographs \cite{tuzoff2019tooth}. Deep learning has also been used to read tooth conditions, not only tooth positions. For example, Oztekin et al. detected dental caries on panoramic radiographs with a convolutional neural network and reported high accuracy \cite{oztekin2023caries}. This shows that both the tooth and its condition can be learned from panoramic images, which is also what this thesis asks the models to do.
...
\begin{itemize}
\item Chapter~1 (Introduction) introduces the research background, motivation, objectives, scope, and overall structure of the thesis.

\item Chapter~2 (Materials and Methods) covers the PAN924 source data, the five-view design, the label set, class imbalance handling, the five vision language models, the report-generation pipeline, and LoRA/QLoRA adaptation.

\item Chapter~3 (Experimental Setup) presents the training environment, the shared model settings, the best-checkpoint selection strategy, and the evaluation metrics.

\item Chapter~4 (Results) reports the per-model scores, the zero-shot baseline, the per-condition and per-tooth analysis, and the comparison with a Faster R-CNN recognition baseline.

\item Chapter~5 (Discussion and Conclusion) interprets the findings, discusses limitations and future work, and concludes the thesis.

\item The appendix lists the prompt templates, and the References close the report.
...
This chapter presents the data and the methods together. The first part, Materials, describes the PAN924 source data and how the report dataset is built from it. The second part, Methods, describes the five models and how each one is adapted for the task.

\section{Materials}
\label{sec:materials}

The materials are the PAN924 dataset and the report dataset built from it. This part covers the raw images, the FDI label set, the data split, the report schema, and how the strong class imbalance is handled.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.8\textwidth]{annotated_example.png}
    \caption{An annotated panoramic image from PAN924. Each tooth has an oriented bounding box and its FDI number. In the VLM dataset, each tooth is reduced to its FDI number and condition label.}
...
The first script in the pipeline is \texttt{build\_5view\_common\_dataset.py}. It reads the original annotations, loads the source images, and builds the common training data. This common dataset is the main dataset of the thesis. The model-specific datasets are created from it later. Figure~\ref{fig:data-pipeline} summarises the full data-preparation pipeline.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        stage/.style={draw, rounded corners, align=center,
                      text width=2.7cm, minimum height=2cm, font=\small},
        >=Stealth, node distance=0.8cm,
    ]
        \node[stage]               (d1) {\textbf{PAN924 data}\\[3pt]{\scriptsize images and\\JSON annotations}};
        \node[stage, right=of d1]  (d2) {\textbf{Clean labels}\\[3pt]{\scriptsize FDI number\\and condition}};
        \node[stage, right=of d2]  (d3) {\textbf{Create five views}\\[3pt]{\scriptsize full image and\\four regional crops}};
        \node[stage, right=of d3]  (d4) {\textbf{Build samples}\\[3pt]{\scriptsize prompt, JSON answer,\\image path}};
        \node[stage, right=of d4]  (d5) {\textbf{Export data}\\[3pt]{\scriptsize VLM JSONL\\and COCO}};
        \draw[->] (d1) -- (d2);
        \draw[->] (d2) -- (d3);
        \draw[->] (d3) -- (d4);
        \draw[->] (d4) -- (d5);
        \node[anchor=west, font=\large\bfseries] (dttl) at ([yshift=1.55cm]d1.north west) {Data Preparation Pipeline};
        \node[anchor=west, font=\small] at ([yshift=1.0cm]d1.north west) {From the PAN924 annotations to the training files.};
        \node[anchor=west, font=\footnotesize] at ([yshift=-0.85cm]d1.south west) {The same source data is reused for both VLM report generation and Faster R-CNN detection.};
    \end{tikzpicture}}
    \caption{The data-preparation pipeline: raw PAN924 images and annotations are cleaned to FDI numbers and condition labels, expanded into five views, built into report-style samples, and exported into the model input formats.}
...
A full adult mouth has 32 teeth, 8 in each quadrant. Many patients do not have all 32. Wisdom teeth (position 8) are often missing, already removed, or still under the gum. This is one reason why some FDI numbers appear less often in the dataset. Figure~\ref{fig:fdi} shows the four quadrants and the numbering.

\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth}{%
    \begin{tikzpicture}[
        q/.style={draw, rounded corners, align=center, minimum width=4cm, minimum height=2cm, font=\small},
    ]
        \node[q] (q1) at (0,0)   {\textbf{Quadrant 1}\\upper right\\FDI 11--18};
        \node[q] (q2) at (5,0)   {\textbf{Quadrant 2}\\upper left\\FDI 21--28};
        \node[q] (q4) at (0,-3)  {\textbf{Quadrant 4}\\lower right\\FDI 41--48};
        \node[q] (q3) at (5,-3)  {\textbf{Quadrant 3}\\lower left\\FDI 31--38};
        \draw[dashed] (2.5,1.4) -- (2.5,-4.4);
        \draw[dashed] (-2.6,-1.5) -- (7.6,-1.5);
        \node[font=\footnotesize\itshape] at (2.5,1.7) {midline};
    \end{tikzpicture}}
    \caption{The FDI tooth numbering system. Quadrants 1--2 are the upper jaw and 3--4 the lower jaw; inside each quadrant the numbers grow from front to back.}
    \label{fig:fdi}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Dental condition labels and counts after merging into 12 classes}
\renewcommand{\arraystretch}{1}
\begin{tabular}{@{}l r p{7cm}@{}}
\toprule
\textbf{Code} & \textbf{Count} & \textbf{Meaning} \\
\midrule
\texttt{H}    & 10,665 & Healthy \\
\texttt{R}    & 6,481  & Restored tooth/filling or crown \\
\texttt{Te}   & 869    & Endodontically treated tooth \\
\texttt{C}    & 399    & Suspected caries \\
\texttt{CpuM} & 384    & Mixed prosthetic crown \\
\texttt{Di}   & 312    & Incisal or occlusal wear \\
\texttt{M3i}  & 304    & Impacted third molar \\
\texttt{M3f}  & 192    & Developing third molar \\
\texttt{Rr}   & 141    & Residual root \\
\texttt{P}    & 127    & Pontic \\
\texttt{Im}   & 90     & Implant \\
\texttt{Dc}   & 69     & Crown destruction \\
\bottomrule
\end{tabular}
\end{table}
...
Crop-level prevalence ($p_c$) is used rather than raw tooth count. One crop can contain several implants, so using the tooth count would give extra weight to crops that happen to have many instances. Prevalence counts each crop once and is a cleaner measure of how common the condition is across the regional crops.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[stage]               (s1) {\textbf{Panoramic image}\\[3pt]{\scriptsize full view or\\regional crop}};
        \node[stage, right=of s1]  (s2) {\textbf{Vision encoder}\\[3pt]{\scriptsize ViT $\rightarrow$\\image tokens}};
        \node[stage, right=of s2]  (s3) {\textbf{Connector}\\[3pt]{\scriptsize MLP into the\\LLM input space}};
        \node[stage, right=of s3]  (s4) {\textbf{Language model}\\[3pt]{\scriptsize reads tokens\\and schema prompt}};
        \node[stage, right=of s4]  (s5) {\textbf{JSON report}\\[3pt]{\scriptsize parsed into\\tooth findings}};
        \draw[->] (s1) -- (s2);
        \draw[->] (s2) -- (s3);
        \draw[->] (s3) -- (s4);
        \draw[->] (s4) -- (s5);
        \node[anchor=west, font=\large\bfseries] (ttl) at ([yshift=1.55cm]s1.north west) {Inference Workflow: From X-ray to Report};
        \node[anchor=west, font=\small] at ([yshift=1.0cm]s1.north west) {A trained model turns a panoramic image into a structured report.};
        \node[anchor=west, font=\footnotesize] at ([yshift=-0.85cm]s1.south west) {Only the vision encoder, connector and language model change between models; the prompt, schema and parsing stay the same.};
    \end{tikzpicture}}
    \caption{The report-generation pipeline: image $\rightarrow$ vision encoder $\rightarrow$ connector $\rightarrow$ language model $\rightarrow$ parsed JSON report. The prompt states the JSON schema to return.}
    \label{fig:pipeline}
\end{figure}
...
Each model is described in short below. The checkpoint name is the exact version used in this project, and the citation points to its paper or model card. The following figures show representative architectures.

\minisection{Qwen2.5-VL-7B-Instruct (Alibaba)}

Checkpoint: \texttt{Qwen/Qwen2.5-VL-7B-Instruct} \cite{qwen25vl2025}. Its vision encoder is a ViT with native dynamic resolution, so the image is not forced to a fixed size and small details are kept. The language model is the Qwen2.5 decoder (about 7B). It is the larger of the two Qwen2.5-VL variants used in this study.

\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth}{%
    \begin{tikzpicture}[
        box/.style={draw, rounded corners, align=center, minimum height=1.3cm, minimum width=2.8cm, font=\small},
        >=Stealth,
    ]
        \node[box] (img) {Image\\(native resolution)};
        \node[box, right=1cm of img] (vit) {Native dynamic\\resolution ViT};
        \node[box, right=1cm of vit] (llm) {Qwen2.5 language\\model ($\sim$7B)};
        \node[box, right=1cm of llm] (out) {JSON report};
        \draw[->] (img) -- (vit);
        \draw[->] (vit) -- (llm);
        \draw[->] (llm) -- (out);
    \end{tikzpicture}}
...
Checkpoint: \texttt{OpenGVLab/InternVL3-8B} \cite{zhu2025internvl3}. It follows a ViT--MLP--LLM design, with an InternViT-300M encoder and a Qwen2.5 language model (about 7B). It cuts a large image into 448-pixel tiles and uses a pixel unshuffle step to keep the token count low, which helps with tiny findings. Figure~\ref{fig:internvl-arch} shows its architecture.

\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth}{%
    \begin{tikzpicture}[
        box/.style={draw, rounded corners, align=center, minimum height=1.3cm, minimum width=2.6cm, font=\small},
        >=Stealth,
    ]
        \node[box] (img) {Image\\(448px tiles)};
        \node[box, right=0.9cm of img] (vit) {InternViT-300M};
        \node[box, right=0.9cm of vit] (conn) {MLP connector\\(pixel unshuffle)};
        \node[box, right=0.9cm of conn] (llm) {Qwen2.5 language\\model ($\sim$7B)};
        \node[box, right=0.9cm of llm] (out) {JSON report};
        \draw[->] (img) -- (vit);
        \draw[->] (vit) -- (conn);
        \draw[->] (conn) -- (llm);
        \draw[->] (llm) -- (out);
    \end{tikzpicture}}
    \caption{Architecture of InternVL3: InternViT-300M and an MLP connector (with pixel unshuffle) feed the Qwen2.5 language model.}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[box] (img) {Image\\(fixed 448px)};
        \node[box, right=0.9cm of img] (vit) {SigLIP-So400m};
        \node[box, right=0.9cm of vit] (conn) {Linear\\projection};
        \node[box, right=0.9cm of conn] (llm) {Gemma~2 language\\model (prefix-LM)};
        \node[box, right=0.9cm of llm] (out) {JSON report};
        \draw[->] (img) -- (vit);
        \draw[->] (vit) -- (conn);
        \draw[->] (conn) -- (llm);
        \draw[->] (llm) -- (out);
    \end{tikzpicture}}
    \caption{Architecture of PaliGemma~2: a SigLIP-So400m encoder and a linear projection feed the Gemma~2 language model (prefix-LM).}
...
The matrix $W$ stays frozen, and only $A$ and $B$ are trained. The term $\alpha/r$ is a fixed scaling factor that sets how strong the update is. Because $r$ is small, $A$ and $B$ together hold far fewer numbers than $W$. This drops the trainable weights from billions to only a few million. Figure~\ref{fig:qlora-arch} illustrates this low-rank update.

\begin{figure}[H]
    \centering
    \adjustbox{max width=\textwidth}{%
    \begin{tikzpicture}[
        box/.style={draw, rounded corners, align=center, font=\small, minimum height=1cm},
        >=Stealth,
    ]
        \node (x) {$x$};
        \node[box, fill=gray!15, right=1.6cm of x] (W) {Frozen weight $W$\\(stored in 4-bit)};
        \node[box, minimum width=1.4cm, below=1.4cm of W] (A) {$A$\\($r\times k$)};
        \node[box, minimum width=1.4cm, right=1.6cm of A] (B) {$B$\\($d\times r$)};
        \node[circle, draw, right=1.8cm of W] (sum) {$+$};
        \node[right=1.4cm of sum] (h) {$h$};
        \draw[->] (x) -- (W);
        \draw[->] (W) -- (sum);
        \draw[->] (x) |- (A);
        \draw[->] (A) -- (B);
        \draw[->] (B) -| (sum);
        \draw[->] (sum) -- (h);
        \node[below=0.25cm of B, font=\footnotesize] {trainable low-rank update $\tfrac{\alpha}{r}BA$};
    \end{tikzpicture}}
    \caption{LoRA adaptation. The weight $W$ is frozen (4-bit in QLoRA); only the low-rank matrices $A$ and $B$ are trained and added to the frozen path.}
...
To counter this, the loss on the tokens that spell a rare condition code is multiplied by a fixed weight (here, three), while every other token keeps a weight of one. The rare codes are the same five used earlier: \texttt{Dc}, \texttt{Im}, \texttt{P}, \texttt{Rr}, and \texttt{M3f}. In practice, the token ids that are distinctive to these codes are read from each model's tokenizer once, with the ids shared with the common codes \texttt{H} and \texttt{R} (such as quotes and colons) removed, and the training loss is scaled up whenever the target token is one of them. The penalty for mislabelling a rare tooth is now three times larger, so the model can no longer ignore these conditions cheaply.
...
A checkpoint is saved every 100 steps, and the last checkpoint is not always the best one. The validation loss is recorded at every save point, so the checkpoint with the lowest validation loss is selected for the final test. This selection is cheap, because the validation loss is already computed during training and needs no extra generation. The chosen checkpoint is the only one run on the test set.
...
This chapter reports the results of the five models on the held-out test set of 465 rows. All models use the same checkpoint-selection rule and the same evaluation script, so the numbers can be compared directly. The per-tooth chart and the confusion matrix are shown for the best model, InternVL3, while the tables cover all five models. A zero-shot baseline, the same models without fine-tuning, is also reported in Section~\ref{sec:zeroshot}.
...
\midrule
Qwen2.5-VL-7B      & \textbf{0.779} & 0.846 & 0.811 & 0.151 & 0.537 & 0.452 \\
...
PaliGemma~2 (3B)   & 0.768 & \textbf{0.901} & 0.829 & 0.174 & 0.606 & 0.518 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
InternVL3-8B       & 0.224 & 0.108 & 0.133 & 0.695 & 100 \\
Phi-3.5-vision     & 0.279 & 0.105 & 0.127 & 0.603 & 99.6 \\
PaliGemma~2 (3B)   & 0.232 & \textbf{0.099} & \textbf{0.135} & \textbf{0.697} & 100 \\
\bottomrule
\end{tabular}
\end{table}

The recognition results split into two clear levels. InternVL3-8B is the best model overall. It has the highest macro F1 (0.189), micro F1 (0.607), and tooth accuracy (0.519). PaliGemma~2 follows very closely, even though it is the smallest model and uses a fixed 448-pixel input. Qwen2.5-VL comes next, and Phi-3.5-vision is the weakest, last on the headline recognition metrics (FDI precision and F1, macro and micro F1, and tooth accuracy).
...
\begin{figure}[H]
    ...
    \caption{Overall recognition metrics for the five models. FDI detection is high and similar for all, while macro F1 stays low: condition classification is the hard part. InternVL3 is best, PaliGemma~2 a close second.}
    \label{fig:model-comparison}
\end{figure}

\section{Zero-Shot Baseline: the Effect of Fine-tuning}
\label{sec:zeroshot}

The numbers above are for the fine-tuned models. To show how much the fine-tuning adds, the two strongest families were also run on the test set \emph{without} their LoRA adapter, that is, as the original base model in a zero-shot way. The setup is otherwise identical: the same test rows, the same schema prompt, greedy decoding, and the same 4-bit base. Only the adapter is removed. Table~\ref{tab:base-vs-ft} compares the two settings.

\begin{table}[H]
\centering
\caption{Zero-shot base model (no fine-tuning) versus the fine-tuned model. The base run uses the same prompt, test set, decoding, and 4-bit base; only the LoRA adapter is removed.}
\label{tab:base-vs-ft}
\small
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{llcccccc}
\toprule
Model & Setting & Valid JSON (\%) & FDI F1 & Micro F1 & Macro F1 & Tooth acc. & ROUGE-L \\
\midrule
\multirow{2}{*}{InternVL3-8B}  & zero-shot base & 93.3 & 0.019 & 0.000 & 0.000 & 0.000 & 0.087 \\
                               & fine-tuned     & 100  & \textbf{0.830} & \textbf{0.607} & \textbf{0.189} & \textbf{0.519} & \textbf{0.695} \\
\midrule
\multirow{2}{*}{Qwen2.5-VL-7B} & zero-shot base & 100  & 0.077 & 0.000 & 0.000 & 0.000 & 0.100 \\
                               & fine-tuned     & 100  & \textbf{0.811} & \textbf{0.537} & \textbf{0.151} & \textbf{0.452} & \textbf{0.654} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

The gap is very large. Without fine-tuning, both base models score essentially zero on recognition. They still produce well-formed JSON (93 to 100\% valid), but the FDI detection F1 is near zero (0.02 to 0.08), and the condition classification and tooth accuracy are exactly zero. In other words, the base model copies the shape of the JSON, but it does not list the right teeth and does not use the right condition labels.

There are two reasons for this. First, the prompt gives the JSON schema, but it does not list the twelve condition codes (\texttt{H}, \texttt{R}, \texttt{Dc}, and so on); those codes are specific to this dataset and are only learned during fine-tuning. Zero-shot, the base model invents its own words (for example \texttt{normal} or \texttt{present}), so no condition matches the gold label. Second, even ignoring the condition, the base model lists very few teeth in the exact region-and-FDI structure that the score requires, so the detection recall is almost zero. The fine-tuned model, in contrast, reaches an FDI F1 around 0.81 to 0.83 and a tooth accuracy around 0.45 to 0.52.

The reading is clear: for this structured dental task, almost all of the measured ability comes from the fine-tuning, not from the base model or the prompt. One caveat should be kept: these numbers are for a zero-shot run with the task prompt. A different prompt, or a few worked examples in the prompt, might lift the base model somewhat, but it would still have to be told the dataset's label set. The result shows that fine-tuning on the PAN924 data is the decisive step.

\section{Per-Condition Results}

Table~\ref{tab:results-percond} reports the per-condition F1 for every model. The support column is the number of tooth instances across all test report rows, including the full-image and the regional-view samples, so one tooth can be counted more than once. It is the same for all models because it comes from the gold labels. The five rare conditions (\texttt{Dc}, \texttt{Im}, \texttt{P}, \texttt{Rr}, \texttt{M3f}) are the ones most affected by the class imbalance, so they are the most important rows to read.
...
\begin{figure}[H]
    ...
    \caption{Per-tooth (FDI) detection recall for the best model (InternVL3). Each cell shows the recall and test count for that tooth position.}
    \label{fig:fdi-chart}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Row-normalised confusion matrix for the best model (InternVL3). The diagonal is per-condition recall; off-diagonal cells are common mistakes.}
    \label{fig:confusion}
\end{figure}
...
\newpage
\section{Web Interface for Report Generation}
\label{sec:interface}

This last section adds no new measurement. It only shows how the best model is packaged into a small web tool, allowing a structured report to be produced from an image without manually running scripts. The interface uses the same model and prompt as in the experiments, so it does not change any result reported above.

The tool is built with FastAPI. After a panoramic X-ray is uploaded, the application runs the fine-tuned InternVL3-8B model with the full-report prompt from Appendix~\ref{app:prompts} and returns the same JSON report format used in the evaluation. This JSON output remains available for direct inspection, while the interface also converts it into a readable page where regions, teeth in FDI notation, conditions, comments, and the summary are displayed clearly with simple colour cues. Figure~\ref{fig:interface} shows the result page for one image.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{interface.png}
    \caption{The web interface for report generation.}
    \label{fig:interface}
\end{figure}

\chapter{Discussion and Conclusion}

This chapter interprets the results, states the main limitations and the future work, and closes the thesis.
...
This leads to the first conclusion: model size is not the main factor here. The 3B PaliGemma~2 matches the 8B InternVL3, and the 3B Qwen matches or slightly beats the 7B Qwen. The 7B Qwen only has a small edge on macro F1, which means it handles the rare conditions a little better. In this controlled setup, increasing the language-model size alone did not consistently improve performance. The architecture and the visual pretraining appear to matter at least as much as the parameter count.

The vision encoder is the most likely reason for the ranking. InternVL3 uses InternViT-300M, and PaliGemma~2 uses SigLIP-So400m. Both are strong vision encoders trained on very large image data, and PaliGemma~2 is also pretrained on grounded tasks such as detection and captioning. These two models take the first and second place. Phi-3.5-vision uses an older CLIP ViT-L/14 encoder, and it is the weakest model on the headline recognition metrics. The two Qwen models sit in the middle and score almost the same, which again shows that the language-model size is not the bottleneck. So on this task, the quality of the vision encoder, more than the parameter count, seems to explain much of the difference between the models. This is a correlation across five models, not an isolated test of the encoder, because the models also differ in their checkpoints, pretraining, and image handling.

The second conclusion is about where the models fail. Tooth detection is easier than condition classification. Every model finds the teeth with an FDI F1 of about 0.80 to 0.83. The problem is the condition. The tooth accuracy drops to 0.38 to 0.52, and the macro F1 stays near 0.10 to 0.19. The main error type is therefore a wrong or missing condition, not an invented tooth. The confusion analysis confirms this: a rare condition is usually predicted as healthy or restored, the two common classes.

The third conclusion is about the class imbalance. The two techniques, oversampling and rare-token loss weighting, do help. They keep several rare conditions above zero and stop the model from labelling every tooth as healthy. But the help is limited. The rarest conditions, crown destruction and residual root, stay near zero, and even caries, which is not rare, is almost never read correctly. This weak point is not unique to this work: a 2025 study reported that even GPT-4o, a much larger proprietary VLM, scored zero F1 on caries on panoramic radiographs \cite{liu2025dentalvlm}, which suggests that caries is hard for current VLMs at this image scale. The handling also has a cost: the model now over-predicts the conditions it was pushed to learn, such as developing third molar. So the imbalance is reduced but not solved.

The Faster R-CNN baseline in Section~\ref{sec:frcnn-compare} shows that this is not only a data limit. A specialist detector reaches a much higher rare-condition score on the same PAN924 source data, although on a different test split and with a different matching rule. So a large part of the gap comes from the report-generation approach itself, where the rare codes are weak in the token-level loss, and not only from the class imbalance.

The zero-shot baseline in Section~\ref{sec:zeroshot} points the same way from the other side. Without fine-tuning, the same base models score near zero on recognition, so the ability measured in this work is produced by the fine-tuning on the dental data, not by the base model or the prompt alone.
...
A few limitations should be kept in mind when reading these results.

\begin{itemize}
    \item \textbf{Rare conditions.} The models are weak on the rare conditions. Crown destruction and residual root stay near zero, and caries is almost never read correctly, so the macro F1 is low.
    \item \textbf{Report text.} The training comments and summaries are built from the ground-truth labels with fixed templates. They are readable, but they are not written by a dentist, and they add no finding beyond the labels.
    \item \textbf{Single data source.} All images come from one dataset (PAN924, 924 images). The results may not transfer to images from other clinics or machines.
    \item \textbf{No expert review.} The reports are scored against the label-based gold report, not judged by a dentist, so a clinical-quality check is still missing.
    \item \textbf{4-bit training.} The models are trained in 4-bit QLoRA on a single GPU. Full-precision training might gain a little, but it was not affordable on this hardware.
\end{itemize}

Several directions follow from these limitations.

\begin{itemize}
    \item \textbf{Better rare-class handling.} Stronger methods, such as a focal loss, harder oversampling, or a detect-then-describe two-stage design, could raise the rare-condition scores.
    \item \textbf{Higher detail for small lesions.} Caries and similar small findings may need a higher input resolution or a lesion-focused crop.
    \item \textbf{More and broader data.} A larger, multi-source dataset, with expert review of the generated reports, would make the evaluation stronger and the model more general.
    \item \textbf{Clinical pilot.} The web interface in Section~\ref{sec:interface} could be tested with a dentist in the loop, to measure how useful the draft report is in practice.
\end{itemize}

\section{Conclusion}

This thesis studied whether vision language models can produce a first-draft dental report from a panoramic X-ray. It built a reusable PAN924-based instruction dataset, with five views per image and report-style targets, fine-tuned five vision language models with one shared QLoRA recipe, and evaluated them with a single script and against a Faster R-CNN recognition baseline.

The main findings are consistent. InternVL3-8B is the best model, with PaliGemma~2 a close second, which suggests that the architecture and the visual pretraining may matter at least as much as the parameter count. Tooth detection is easier than naming the condition for every model: the condition is the hard part, the rare conditions remain difficult, and caries is a separate small-lesion failure case. The zero-shot baseline shows that almost all of this ability comes from the fine-tuning, not from the base model. The Faster R-CNN baseline reaches a higher recognition score, while the vision language models add the readable, structured report that a detector cannot produce.

The overall answer is therefore cautious but positive. Vision language models can already produce a useful first-draft dental report for the common findings, but the report still needs a dentist to check it, and the rare-condition recognition needs more work. The clearest next step is to strengthen the rare-condition recognition, through stronger class-imbalance methods, higher input detail for the small lesions, and a larger, multi-source dataset with expert review.
