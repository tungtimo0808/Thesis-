...
This last section adds no new measurement. It shows how the best model is packaged into a small tool, so the structured report can be produced from an image without running any script by hand. The interface is a thin layer on top of the same model and the same prompt used in the experiments; it does not change anything that was measured above.

The tool is a web application built with FastAPI. The user uploads a panoramic X-ray, the application runs the fine-tuned InternVL3-8B model with the full-report prompt from Appendix~\ref{app:prompts}, and the model returns the same JSON report used in the evaluation. The raw output therefore stays measurable and identical in form to the test predictions. The application then turns that JSON into a readable page: each region is listed with its teeth in FDI notation, the condition of each tooth is shown with a colour cue (healthy, restored, or abnormal), and the short comments and summary are kept as plain text. The original JSON is still available behind a toggle, so the structured output can be checked directly.

Figure~\ref{fig:interface} shows the result page for one panoramic image. The page is intentionally simple and keeps a clear warning at the top: the output is a draft, and a qualified dentist must review and confirm every finding before any clinical use. This matches the framing of the whole thesis, where the generated report is a first draft and not a final diagnosis.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{interface.png}
    \caption{The web interface for report generation. The uploaded panoramic X-ray is shown on the left, and the report produced by the fine-tuned InternVL3-8B model is rendered on the right: each tooth is listed by its FDI number with a colour-coded condition, followed by the region comments and the overall summary. A warning makes clear that the report is a draft for dentist review.}
    \label{fig:interface}
\end{figure}
...
The main findings are consistent. InternVL3-8B is the best model, with PaliGemma~2 a close second, which shows that the vision encoder matters more than the parameter count. Detection of the teeth is easy for every model, while naming the condition is the hard part, and the rare conditions, especially caries, stay the open problem. The zero-shot baseline shows that almost all of this ability comes from the fine-tuning, not from the base model. The Faster R-CNN baseline reaches a higher recognition score, while the vision-language models add the readable, structured report that a detector cannot produce.
