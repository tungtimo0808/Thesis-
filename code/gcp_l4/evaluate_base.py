...
This is the same evaluation as evaluate.py, but it runs the original pretrained model
WITHOUT the fine-tuned adapter. Comparing these numbers against the fine-tuned ones shows
how much the QLoRA fine-tuning actually added.

Note: the base models were never shown the dental condition codes (Dc, Rr, M3f, ...), so
the condition classification scores are expected to be very low. The detection (FDI) score
may be partly there, since an instruction-tuned model can still follow the JSON schema and
read tooth numbers. Some base models may also produce fewer valid-JSON outputs.

Usage:
  python evaluate_base.py qwen        # one of: qwen | qwen3b | internvl | phi | paligemma
  python evaluate_base.py internvl

Outputs (written next to the model's run folder, with a "_base" suffix so they never
overwrite the fine-tuned results):
  ~/pan924_runs/<key>/infer_test_base.jsonl     raw swift predictions
  ~/pan924_runs/<key>/preds_test_base.jsonl      adapted {id, prediction}
  ~/pan924_runs/<key>/metrics_<key>_base.json    metrics
  ~/pan924_runs/<key>/report_<key>_base.txt       human-readable report
"""
import json
import os
import sys
