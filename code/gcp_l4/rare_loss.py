...
Counters the majority bias (model relabelling rare teeth as H/R) at the loss level, without
duplicating H/R the way oversampling does. Per-token CE is multiplied by RARE_LOSS_WEIGHT where the
(shifted) label token id belongs to the rare set, and by 1.0 elsewhere.

Enabled by train.py --rare-loss, which:
  - computes the distinctive rare token ids for the model and exports RARE_TOKEN_IDS (csv),
  - exports RARE_LOSS_WEIGHT,
  - runs:  swift sft ... --external_plugins rare_loss.py --loss_type rare_weighted

Registration: swift's loss-plugin API name moved around across versions, so instead of guessing one
import we write the loss DIRECTLY into swift's loss mapping (trying every known dict/function form).
On import as an external plugin it prints `[rare_loss] registered "rare_weighted": True`.

Run `python rare_loss.py` for a standalone numeric self-test (no swift needed).
"""
import os
...
def _register():
    ...
    Robust across ms-swift versions: tries the register_loss_func() API (both the 2-arg and the
    decorator form) AND direct assignment into the loss mapping dict (loss_mapping / LOSS_MAPPING)."""
    try:
...
def _allow_local_resume():
    ...
    so no-op the check — this lets training RESUME after a crash/preemption without upgrading torch
    (upgrading torch breaks the working bitsandbytes/CUDA stack on this VM)."""
    import importlib
