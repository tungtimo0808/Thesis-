...
generation-bound step), scores them with eval_report.py, and writes both a machine-readable JSON and
a human-readable TXT report next to the run.

Usage:
  python evaluate.py qwen                       # uses <output_dir>/best_checkpoint.txt
  python evaluate.py qwen --checkpoint <path>   # force a specific checkpoint
"""
import json
import os
import sys
...
def main():
    ...
    # re-score OLD predictions (this exact bug bit the Colab run).
